    ...
    def run(self):
        ...
        while world_state.is_mission_running:
            ...
            if world_state.number_of_observations_since_last_state > 0:
                ...
                #Ha az előző cselekvés végrehajtódott akkor jöhet csak a következő, különben várakozik.
                if (lastX!=self.x or lastY!=self.y or lastZ!=self.z or lastYAW!=self.yaw) and self.pitch==45:
                    self.agent_host.sendCommand( "look 1") #Cselekvés közben lefele néz, várakozó álapotban előre.
                    time.sleep(FirstDelay)
                    lastX=self.x
                    lastY=self.y
                    lastZ=self.z
                    ...
                    if Jobb1== "red_flower" or Jobb2== "red_flower" or Jobb3== "red_flower":
                        ...
                        #Ha nem történt szintlelépés, akkor forul vagy tovább megy.
                        if NextBlock=="dirt":
                            self.agent_host.sendCommand("move -1")
                            time.sleep(FirstDelay)
                            self.agent_host.sendCommand( arra )
                            ...
                            for i in range(3):
                                ...
                                time.sleep(FirstDelay)
                    time.sleep(FirstDelay)
                    self.agent_host.sendCommand( "look -1")#Cselekvés vége, várakozó állapotban előre néz.
